 ...
 * 
 * For this version I use floating point weights, and even allow negative
 * values. Though I hope that these individuals will not survive.
 * 
 ...
 *
 */
public class DifferentialEvolution2D {

	public static final int WEIGHT_SIZE = 10;
	public static final int N_GAMES = 6;
	public static final int MAX_GENERATIONS = 1000;
	public static final double MAX_WEIGHT = Double.POSITIVE_INFINITY; // TODO
																		// Test
																		// different
																		// values
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < POP_SIZE; i++) {
			...
			newWeights[0] = 1; // Fix one variable.
			newWeights[1] = random.nextDouble();
			newWeights[2] = 0;
		...
		for (int i = 0; i < MAX_GENERATIONS; i++) {
			...
			for (int x = 0; x < POP_SIZE; x++) {
				...
				double[] newWeights = population.get(x).clone(); // shallow copy
																	// but good
																	// enough
				...
				/*
				 * for each weight change if : -uniformly distributed number
				 * smaller than cross over probability -weight was previously
				 * selected to change.
				 */
				double newW = indiv1[1] + DIFF_WEIGHT * (indiv2[1] - indiv3[1]);
	...
	public static boolean isNewBetter(int n_games) throws IOException {
		...
		// Debug output
		statsOut.println("Old weights WINS (P1): " + Integer.toString(stats[0]));
		statsOut.println("New weights WINS (P2): " + stats[1]);
		statsOut.println("Draws: " + stats[2]);
		statsOut.println("Moves Played When Lost: " + stats[3] + " vs " + stats[4]);
		...
		return newerIsBetter; // Never is only better if more wins or survived
								// longer!
	}
	...
	public static int[] readLogs() throws IOException {
		...
		int[] stats = { 0, 0, 0, 0, 0 }; // WINS P1, WINS P2, DRAWS, P1 MOVES
											// WHEN LOST, P2 MOVES WHEN LOST
		for (int i = 0; i < N_GAMES; i++) {
